...
- Semantic Kernel 集成成功，支持基本 LLM 調用
- 單元測試覆蓋率 ≥ 80%
...
### S1-6: Agent Service - Semantic Kernel Integration
**Story Points**: 8  
**優先級**: P0 - Critical  
...
#### 描述
集成 Microsoft Semantic Kernel SDK，實現 Agent 的 LLM 推理能力和插件架構。

#### 驗收標準
- [ ] Semantic Kernel SDK 集成成功
- [ ] Azure OpenAI 連接配置
- [ ] 實現基本的 LLM 調用功能
- [ ] 支持插件 (Plugins) 架構
- [ ] 實現 Prompt 模板管理
...
class AgentService:
    def __init__(self, db: Session):
        self.db = db
        self.sk_service = SemanticKernelService({
    ...
    async def execute_agent(
        ...
        # 執行 LLM 調用
        result = await self.sk_service.execute_prompt(
...
#### 子任務
1. [ ] 安裝 Semantic Kernel SDK
2. [ ] 配置 Azure OpenAI 連接
3. [ ] 實現 SemanticKernelService 類
4. [ ] 實現 AgentService 類
5. [ ] 實現 Prompt 模板渲染
6. [ ] 實現 LLM 成本計算
7. [ ] 實現插件架構基礎
...
def mock_llm_service(monkeypatch):
    ...
    monkeypatch.setattr("app.services.agent_service.SemanticKernelService", lambda x: mock_service)
    return mock_service
...
#### 風險 1: Semantic Kernel 學習曲線陡峭
- **嚴重性**: 🔴 高
- **概率**: 🟡 中
- **緩解**:
  - Sprint 第一週安排 2 天專門學習
  - 創建內部 Semantic Kernel 使用文檔
